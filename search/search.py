...
def depthFirstSearch(problem):
    ...
    fringe.push(problem.getSuccessors(problem.getStartState()))  # Adds the fringe to the stack

    explored = util.Stack() # Explored to empty

    while not fringe.isEmpty():
        if fringe.isEmpty():
            print("failure") # Has been unable to find the goal state
            return
        node = fringe.pop()
        if problem.isGoalState(node):
            print("success") # Has found the goal state
            return explored
        if not problem.isGoalState(node):
            explored.push(node) # Adds the node to the explored list
            fringe.push(problem.getSuccessors(node)) # Add successors to the fringe
